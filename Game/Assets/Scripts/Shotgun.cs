...
public class Shotgun : Weapon
{
    ...
    private void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.H) && !isShooting && currentAmmo > 0 && !isReloading && !player.isRunning)
        {
            Shoot();
            OnAttack?.Invoke();
            recoilSystem.RecoilFire();
            kickbackSystem.RecoilFire();
        }
        else if(Input.GetKeyDown(KeyCode.H) && !isShooting && currentAmmo == 0 && !isReloading && !player.isRunning)
        {
            OnEmptyClip?.Invoke();
        }
    ...
    private IEnumerator Shooting()
    {
        ...
        for (int i = 0; i < shootCount; i++)
        {
            ...
            if(Physics.Raycast(shootPoint.position, direction, out hit, maxDistance))
            {
                ...
                if(hit.transform.gameObject.GetComponentInParent<BasicSoliderAI>())
                {
        ...
        animator.SetTrigger("PutBullets");
